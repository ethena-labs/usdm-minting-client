...
export const OrderRequest = z.object({
  ...
  benefactor: Address.describe('Address where tokens are taken from (collateral for MINT, USDm for REDEEM)'),
  beneficiary: Address.describe('Address where tokens are received (USDm for MINT, collateral for REDEEM)'),
  collateral_asset: Address.describe('Address of the collateral token (USDC)'),
  collateral_amount: z
    .string()
    .describe('Amount of collateral tokens (as string to handle large numbers)'),
  usdm_amount: z
    .string()
    .describe('Amount of USDm tokens (as string to handle large numbers)'),
})
