 ...
 * Points to the actual USDm API endpoint.
 */
function getServerUrl(): string {
  // Local development
  if (!process.env.VERCEL_ENV) {
    return 'https://public.api.megausd.money'
  }
  ...
  if (branch === 'dev' || process.env.VERCEL_ENV === 'development') {
    return 'https://public.api.staging.megausd.money'
  }
...
function transformSpec() {
  ...
  return {
    openapi: '3.0.3',
    info: nested.info,
    servers: [{ url: getServerUrl(), description: 'USDm Minting API' }],
    tags: nested.tags,
